...
export const api = {
  ...
  listJobs: ({ page = 1, limit = 20, status, search, type }: JobListFilters = {}) => {
    const query = toQueryString({ page, limit, status, search, type });
    return request<PaginatedResult<Job>>(`/jobs?${query}`);
  },
  ...
  cancelJob: (id: string) => request<Job>(`/jobs/${id}/cancel`, { method: "PATCH" }),
...
export type SseCallback = <T extends SseEventType>(event: T, data: SseEventPayload[T]) => void;
