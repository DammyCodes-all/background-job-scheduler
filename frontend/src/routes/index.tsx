...
import { AlertTriangle, FlowIcon, ArrowRightIcon } from "@hugeicons/core-free-icons";
...
const statusConfig: Record<StatusKey, { label: string; value: StatusKey; color: string }> = {
  pending: {
    label: "Pending",
    value: "pending",
    color: "text-status-pending bg-status-pending/20",
  },
  processing: {
    label: "Processing",
    value: "processing",
    color: "text-status-processing bg-status-processing/20",
  },
  completed: {
    label: "Completed",
    value: "completed",
    color: "text-status-completed bg-status-completed/20",
  },
  failed: { label: "Failed", value: "failed", color: "text-status-failed bg-status-failed/20" },
  cancelled: {
    label: "Cancelled",
    value: "cancelled",
    color: "text-status-cancelled bg-status-cancelled/20",
  },
};
...
function DashboardPage() {
  ...
  const { data: recentData } = useQuery({
    ...
    queryFn: () => api.listJobs({ page: 1, limit: 10 }),
          ...
          return (
            <Card key={key}>
              ...
              <CardContent className="space-y-2">
                <div
                  className={`text-2xl font-bold tabular-nums tracking-tight ${config.color.split(" ")[0]}`}
                >
                  {count}
                </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-lg border border-border bg-card px-4 py-3 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          {dlqCount > 0 ? (
            <Link
              to="/dlq"
              className="inline-flex items-center gap-1 font-semibold tabular-nums text-status-failed hover:underline"
            >
        ...
        <div className="flex items-center gap-2">
          <HugeiconsIcon
            icon={FlowIcon}
            strokeWidth={2}
            className="size-3 text-status-processing motion-safe:animate-pulse"
          />
